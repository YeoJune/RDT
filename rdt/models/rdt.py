...
class RDT(nn.Module):
    ...
    def compute_loss(self, input_tokens, targets, attention_mask, loss_masks, gate_targets, chain_lengths,
        ...
        gate_pred, pooled = self.gate(h_0, attention_mask)
        
        # Gate Loss 0
        gate_loss_0 = gate_criterion(gate_pred, gate_targets[:, 0].unsqueeze(1))
        ...
        for step_idx in range(max_chain_len):
            ...
            # Forward Step
            hidden, gate_pred, pooled = self.forward_step(
                hidden,
                attention_mask=attention_mask,
                last_gate_score=gate_pred,
                last_pooled=pooled,
            ...
            # Valid mask 적용
            step_loss_mask = step_loss_mask & valid_mask.unsqueeze(1)
            ...
            if valid_mask.any():
                ...
                gate_loss = torch.tensor(0.0, device=input_tokens.device)
            ...
            if loss_weight_aux > 0:
                ...
                if aux_bs > 0 and valid_mask[:aux_bs].any():
                    ...
                    with torch.no_grad():
                        ...
                        target_dist = torch.softmax(logits_GT / aux_temp, dim=-1)
