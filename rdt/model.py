...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, x): # (Optional for convenience)
        x = self.decoder(x)
...
class RDT(nn.Module):
    ...
    def forward(self, x, attention_mask=None, is_first_step=True):
        # Returns: hidden, gate_pred
        if is_first_step:
            x = self.token_embedding(x) * math.sqrt(self.d_model)
            x = self.pos_encoding(x) # [수정] pos_ids 제거, 자동 적용
    ...
    def inference(self, x, max_steps=20, threshold=0.1):
        self.eval()
        with torch.no_grad():
            hidden, gate_pred = self.forward(x, is_first_step=True)
            step = 1
